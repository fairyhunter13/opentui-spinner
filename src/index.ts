import {
  ...
  type RenderLib,
  resolveRenderLib,
} from "@fairyhunter13/opentui-core"
...
export interface SpinnerOptions extends Omit<
...
export class SpinnerRenderable extends Renderable {
  ...
  private _encodedFrames: Record<string, ReturnType<typeof OptimizedBuffer.prototype.encodeUnicode>> = {}
  ...
  constructor(ctx: RenderContext, options: SpinnerOptions) {
    ...
    this._frames = this._name ? spinners[this._name].frames : (options.frames ?? this._defaultOptions.frames)
    this._interval = this._name ? spinners[this._name].interval : (options.interval ?? this._defaultOptions.interval)
    this._autoplay = options.autoplay ?? this._defaultOptions.autoplay
    this._backgroundColor = options.backgroundColor ?? this._defaultOptions.backgroundColor
  ...
  public set name(value: SpinnerName | undefined) {
    this._freeFrames()
    this._name = value
    this._frames = this._name ? spinners[this._name].frames : this._defaultOptions.frames
    this._interval = this._name ? spinners[this._name].interval : this._defaultOptions.interval
    ...
    for (let i = 0; i < encodedFrame.data.length; i++) {
      const color =
        typeof this._color === "function"
          ? this._color(this._currentFrameIndex, i, this._frames.length, encodedFrame.data.length)
          : this._color

      buffer.drawChar(encodedFrame.data[i].char, x, this.y, parseColor(color), parseColor(this._backgroundColor))
